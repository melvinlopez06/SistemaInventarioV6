        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //si los roles no estan creados, se crearan la primera vez que se registre un usuario
                    ...
                    //  FIN DE CREACION DE ROLES DE USUARIO


                    //  CREANDO COMO ADMIN LOS PRIMEROS USUARIOS
                    //  COMENTAR LUEGO DE CREAR LOS USUARIOS ADMIN RESPECTIVOS
                    //  await _userManager.AddToRoleAsync(user, DS.Role_Admin);
                    //  FIN CREACION DE PRIMEROS USUARIOS

                    if(user.Role == null)    //el valor que recibe desde la pagina de registro
                    {
                        //cuando se crea desde la pagina de registro, el rol viene como null, por tanto se asume que es un cliente
                        await _userManager.AddToRoleAsync(user, DS.Role_Cliente);
                    }
                    else
                    {
                        await _userManager.AddToRoleAsync(user, user.Role);  //lo que se selecciono en la lista de Role
                    }
                    ...
                    await _emailSender.SendEmailAsync(Input.Email, "Confirma tu email",
                        $"Confirma tu cuenta <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>Haz clic aqui</a>.");
                    ...
                    {
                        //cuando el usuario de tipo cliente se registre que se quede logueado de una vez
                        if(user.Role == null)
                        {
                        ...
                        {
                            //el administrador esta creando un usuario, se debe mantener con la sesión de admin iniciada
                            //y solo redireccionar al index de la gestión de usuarios
